...
和WebAssembly端是一套源码。因为没跨域限制，多了一个设置镜像的功能，方便从不同静态托管站请求静态资源。Android上有额外的权限申请。

~~PC端有生成RCC和RSS的功能。RCC是包含QML的，不用重新编译wasm即可更新内容，wasm只负责请求加载即可，所以这依旧是静态的，需要提交并推送。~~

这也是数据统计里Other比较多的原因之一。
...
xml树形编辑，虽然也支持预览，但是对移动端和小屏设备不够方便。

![xmltree](assets/xmltree.jpeg)

## [xml预览](https://uwillno.com?func=xmllist)

xml列表预览，uwrss可以指定query的版本。

![xmllist](assets/xmllist.jpeg)

## [UWrss](https://uwillno.com?func=uwrss)

备用文章更新方案。额外加入`content`、`content_b64`、`source`方便md预览功能路由。可以指定源作为通用rss解析。

## 控件重写

4.4之后，对已使用的控件进行调整了重写，基于Template与Basic，配色与回退依旧采用Material Style。
...
| UWrss    | source=rss链接                                         | 无                      | [?func=uwrss&source=https://uwillno.com/uwrss.xml](https://uwillno.com/?func=uwrss&source=https://uwillno.com/uwrss.xml) | cors                                     |
...
实测部分用例显示异常，经测试似乎除了C#版本，其它版本也存在类似问题，官方Skeleton Viewer（Java）、WebGL和其它cpp版本都显示异常，又是比较老的分支，修复应该是不太可能了。加入4.2支持。
...
- [cpp_blind_watermark](https://github.com/jiemojiemo/cpp_blind_watermark)
