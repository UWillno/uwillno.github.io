  ...
  return async function (moduleArg = {}) {
    ...
    async function createWasm() {
      function receiveInstance(instance, module) {
        ...
        registerTLSInit(wasmExports["hj"]);
    ...
    function ___syscall_faccessat(dirfd, path, amode, flags) {
      if (ENVIRONMENT_IS_PTHREAD)
        return proxyToMainThread(7, 0, 1, dirfd, path, amode, flags);
      path >>>= 0;
      try {
        path = SYSCALLS.getStr(path);
    ...
    function ___syscall_poll(fds, nfds, timeout) {
      if (ENVIRONMENT_IS_PTHREAD)
        return proxyToMainThread(24, 0, 1, fds, nfds, timeout);
      fds >>>= 0;
      try {
        var nonzero = 0;
    ...
    function ___syscall_readlinkat(dirfd, path, buf, bufsize) {
      if (ENVIRONMENT_IS_PTHREAD)
        return proxyToMainThread(25, 0, 1, dirfd, path, buf, bufsize);
      path >>>= 0;
      buf >>>= 0;
      bufsize >>>= 0;
      try {
    ...
    function ___syscall_recvfrom(fd, buf, len, flags, addr, addrlen) {
      if (ENVIRONMENT_IS_PTHREAD)
        return proxyToMainThread(26, 0, 1, fd, buf, len, flags, addr, addrlen);
      buf >>>= 0;
      len >>>= 0;
      addr >>>= 0;
      addrlen >>>= 0;
    ...
    function ___syscall_recvmsg(fd, message, flags, d1, d2, d3) {
      if (ENVIRONMENT_IS_PTHREAD)
        return proxyToMainThread(27, 0, 1, fd, message, flags, d1, d2, d3);
      message >>>= 0;
      try {
        var sock = getSocketFromFD(fd);
    ...
    function ___syscall_sendmsg(fd, message, flags, d1, d2, d3) {
      if (ENVIRONMENT_IS_PTHREAD)
        return proxyToMainThread(30, 0, 1, fd, message, flags, d1, d2, d3);
      message >>>= 0;
      d1 >>>= 0;
      d2 >>>= 0;
      try {
    ...
    function ___syscall_socket(domain, type, protocol) {
      if (ENVIRONMENT_IS_PTHREAD)
        return proxyToMainThread(31, 0, 1, domain, type, protocol);
      try {
    ...
    function ___syscall_symlinkat(target, dirfd, linkpath) {
      if (ENVIRONMENT_IS_PTHREAD)
        return proxyToMainThread(33, 0, 1, target, dirfd, linkpath);
      target >>>= 0;
      linkpath >>>= 0;
      try {
    ...
    function ___syscall_truncate64(path, length) {
      if (ENVIRONMENT_IS_PTHREAD)
        return proxyToMainThread(34, 0, 1, path, length);
      path >>>= 0;
      length = bigintToI53Checked(length);
      try {
        if (isNaN(length)) return 61;
    ...
    function ___syscall_unlinkat(dirfd, path, flags) {
      if (ENVIRONMENT_IS_PTHREAD)
        return proxyToMainThread(35, 0, 1, dirfd, path, flags);
      path >>>= 0;
      try {
        path = SYSCALLS.getStr(path);
        path = SYSCALLS.calculateAt(dirfd, path);
    ...
    function ___syscall_utimensat(dirfd, path, times, flags) {
      if (ENVIRONMENT_IS_PTHREAD)
        return proxyToMainThread(36, 0, 1, dirfd, path, times, flags);
      path >>>= 0;
      times >>>= 0;
      try {
        path = SYSCALLS.getStr(path);
    ...
    function __emscripten_notify_mailbox_postmessage(
      ...
      if (targetThread == currThreadId) {
        ...
        postMessage({ targetThread: targetThread, cmd: "checkMailbox" });
      } else {
    ...
    function __munmap_js(addr, len, prot, flags, fd, offset) {
      if (ENVIRONMENT_IS_PTHREAD)
        return proxyToMainThread(38, 0, 1, addr, len, prot, flags, fd, offset);
      addr >>>= 0;
      len >>>= 0;
      offset = bigintToI53Checked(offset);
      try {
    ...
    function __setitimer_js(which, timeout_ms) {
      if (ENVIRONMENT_IS_PTHREAD)
        return proxyToMainThread(39, 0, 1, which, timeout_ms);
      if (timers[which]) {
    ...
    function _alBufferData(bufferId, format, pData, size, freq) {
      if (ENVIRONMENT_IS_PTHREAD)
        return proxyToMainThread(40, 0, 1, bufferId, format, pData, size, freq);
      pData >>>= 0;
      if (!AL.currentCtx) {
        return;
      }
        ...
        switch (format) {
          case 4352:
            if (size > 0) {
              ...
              for (var i = 0; i < size; ++i) {
                channel0[i] = GROWABLE_HEAP_U8()[pData++ >>> 0] * 0.0078125 - 1;
              }
            }
          ...
          case 4353:
            if (size > 0) {
              ...
              for (var i = 0; i < size >> 1; ++i) {
                channel0[i] =
                  GROWABLE_HEAP_I16()[pData++ >>> 0] * 30517578125e-15;
              }
            }
          ...
          case 4354:
            if (size > 0) {
              ...
              for (var i = 0; i < size >> 1; ++i) {
                channel0[i] = GROWABLE_HEAP_U8()[pData++ >>> 0] * 0.0078125 - 1;
                channel1[i] = GROWABLE_HEAP_U8()[pData++ >>> 0] * 0.0078125 - 1;
              }
            }
            buf.bytesPerSample = 1;
            buf.channels = 2;
    ...
    function _alDeleteBuffers(count, pBufferIds) {
      if (ENVIRONMENT_IS_PTHREAD)
        return proxyToMainThread(41, 0, 1, count, pBufferIds);
      pBufferIds >>>= 0;
      if (!AL.currentCtx) {
        return;
      }
      for (var i = 0; i < count; ++i) {
        var bufId = GROWABLE_HEAP_I32()[((pBufferIds + i * 4) >>> 2) >>> 0];
        if (bufId === 0) {
          continue;
        }
        if (!AL.buffers[bufId]) {
    ...
    function _alSourcei(sourceId, param, value) {
      if (ENVIRONMENT_IS_PTHREAD)
        return proxyToMainThread(43, 0, 1, sourceId, param, value);
      switch (param) {
        case 514:
        case 4097:
        case 4098:
    ...
    function _alDeleteSources(count, pSourceIds) {
      if (ENVIRONMENT_IS_PTHREAD)
        return proxyToMainThread(42, 0, 1, count, pSourceIds);
      pSourceIds >>>= 0;
      if (!AL.currentCtx) {
        return;
      }
      for (var i = 0; i < count; ++i) {
        var srcId = GROWABLE_HEAP_I32()[((pSourceIds + i * 4) >>> 2) >>> 0];
    ...
    function _alGenBuffers(count, pBufferIds) {
      if (ENVIRONMENT_IS_PTHREAD)
        return proxyToMainThread(44, 0, 1, count, pBufferIds);
      pBufferIds >>>= 0;
      if (!AL.currentCtx) {
        return;
      }
      for (var i = 0; i < count; ++i) {
        ...
        GROWABLE_HEAP_I32()[((pBufferIds + i * 4) >>> 2) >>> 0] = buf.id;
      }
    }
    function _alGenSources(count, pSourceIds) {
      if (ENVIRONMENT_IS_PTHREAD)
        return proxyToMainThread(45, 0, 1, count, pSourceIds);
      pSourceIds >>>= 0;
      if (!AL.currentCtx) {
        return;
      }
      for (var i = 0; i < count; ++i) {
        ...
        GROWABLE_HEAP_I32()[((pSourceIds + i * 4) >>> 2) >>> 0] = src.id;
      }
    }
    ...
    function _alGetSourcei(sourceId, param, pValue) {
      if (ENVIRONMENT_IS_PTHREAD)
        return proxyToMainThread(47, 0, 1, sourceId, param, pValue);
      pValue >>>= 0;
      ...
      switch (param) {
        ...
        case 8202:
        case 53248:
          GROWABLE_HEAP_I32()[(pValue >>> 2) >>> 0] = val;
          break;
        default:
          AL.currentCtx.err = 40962;
          return;
    ...
    function _alGetString(param) {
      if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(48, 0, 1, param);
    ...
    function _alIsExtensionPresent(pExtName) {
      if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(49, 0, 1, pExtName);
      pExtName >>>= 0;
    ...
    function _alSourceQueueBuffers(sourceId, count, pBufferIds) {
      if (ENVIRONMENT_IS_PTHREAD)
        return proxyToMainThread(52, 0, 1, sourceId, count, pBufferIds);
      pBufferIds >>>= 0;
      if (!AL.currentCtx) {
        return;
      }
    ...
    function _alSourceRewind(sourceId) {
      if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(53, 0, 1, sourceId);
      if (!AL.currentCtx) {
        return;
      }
      var src = AL.currentCtx.sources[sourceId];
    ...
    function _alSourceUnqueueBuffers(sourceId, count, pBufferIds) {
      if (ENVIRONMENT_IS_PTHREAD)
        return proxyToMainThread(55, 0, 1, sourceId, count, pBufferIds);
      pBufferIds >>>= 0;
      if (!AL.currentCtx) {
        return;
      }
    ...
    function _alSourcef(sourceId, param, value) {
      if (ENVIRONMENT_IS_PTHREAD)
        return proxyToMainThread(56, 0, 1, sourceId, param, value);
      switch (param) {
        case 4097:
        case 4098:
        case 4099:
    ...
    function _alcCaptureSamples(deviceId, pFrames, requestedFrameCount) {
      ...
      function setF32Sample(i, sample) {
        GROWABLE_HEAP_F32()[((pFrames + 4 * i) >>> 2) >>> 0] = sample;
      }
      function setI16Sample(i, sample) {
        GROWABLE_HEAP_I16()[((pFrames + 2 * i) >>> 1) >>> 0] = sample;
      }
    ...
    function _alcCaptureStop(deviceId) {
      if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(61, 0, 1, deviceId);
      deviceId >>>= 0;
    ...
    function _alcCreateContext(deviceId, pAttrList) {
      if (ENVIRONMENT_IS_PTHREAD)
        return proxyToMainThread(63, 0, 1, deviceId, pAttrList);
      deviceId >>>= 0;
      pAttrList >>>= 0;
    ...
    function _alcDestroyContext(contextId) {
      if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(64, 0, 1, contextId);
      contextId >>>= 0;
    ...
    function _alcGetIntegerv(deviceId, param, size, pValues) {
      if (ENVIRONMENT_IS_PTHREAD)
        return proxyToMainThread(66, 0, 1, deviceId, param, size, pValues);
      deviceId >>>= 0;
      pValues >>>= 0;
      if (size === 0 || !pValues) {
        return;
      ...
      switch (param) {
        case 4096:
          GROWABLE_HEAP_I32()[(pValues >>> 2) >>> 0] = 1;
          break;
        case 4097:
          GROWABLE_HEAP_I32()[(pValues >>> 2) >>> 0] = 1;
          break;
        case 4098:
          ...
          GROWABLE_HEAP_I32()[(pValues >>> 2) >>> 0] =
            AL.currentCtx.attrs.length;
          break;
        case 4099:
          if (!(deviceId in AL.deviceRefCounts)) {
            AL.alcErr = 40961;
          ...
          for (var i = 0; i < AL.currentCtx.attrs.length; i++) {
            GROWABLE_HEAP_I32()[((pValues + i * 4) >>> 2) >>> 0] =
              AL.currentCtx.attrs[i];
          }
          break;
        case 4103:
          if (!(deviceId in AL.deviceRefCounts)) {
          ...
          GROWABLE_HEAP_I32()[(pValues >>> 2) >>> 0] =
            AL.currentCtx.audioCtx.sampleRate;
          break;
        case 4112:
        case 4113:
          if (!(deviceId in AL.deviceRefCounts)) {
          ...
          GROWABLE_HEAP_I32()[(pValues >>> 2) >>> 0] = 2147483647;
          break;
        case 6546:
        case 6547:
          if (!(deviceId in AL.deviceRefCounts)) {
          ...
          GROWABLE_HEAP_I32()[(pValues >>> 2) >>> 0] = hrtfStatus;
          break;
        case 6548:
          if (!(deviceId in AL.deviceRefCounts)) {
            AL.alcErr = 40961;
        ...
        case 786:
          ...
          GROWABLE_HEAP_I32()[(pValues >>> 2) >>> 0] = nsamples;
          break;
        default:
          AL.alcErr = 40963;
          return;
    ...
    function _alcGetString(deviceId, param) {
      if (ENVIRONMENT_IS_PTHREAD)
        return proxyToMainThread(67, 0, 1, deviceId, param);
      deviceId >>>= 0;
    ...
    function _alcOpenDevice(pDeviceName) {
      if (ENVIRONMENT_IS_PTHREAD)
        return proxyToMainThread(69, 0, 1, pDeviceName);
      pDeviceName >>>= 0;
      if (pDeviceName) {
    ...
    var GL = {
      ...
      getSource: (shader, count, string, length) => {
        ...
        for (var i = 0; i < count; ++i) {
          var len = length
            ? GROWABLE_HEAP_U32()[((length + i * 4) >>> 2) >>> 0]
            : undefined;
          source += UTF8ToString(
            GROWABLE_HEAP_U32()[((string + i * 4) >>> 2) >>> 0],
            len
          );
        }
        return source;
    ...
    function _glBufferData(target, size, data, usage) {
      ...
      GLctx.bufferData(
        target,
        data
          ? GROWABLE_HEAP_U8().subarray(data >>> 0, (data + size) >>> 0)
          : size,
        usage
      );
    }
    ...
    var _glClearBufferfi = (x0, x1, x2, x3) =>
    ...
    function _glCompressedTexImage2D(
      ...
      GLctx.compressedTexImage2D(
        ...
        height,
        border,
        GROWABLE_HEAP_U8().subarray(data >>> 0, (data + imageSize) >>> 0)
      );
    }
    ...
    var __glGetActiveAttribOrUniform = (
      ...
      if (info) {
        ...
        if (length)
          GROWABLE_HEAP_I32()[(length >>> 2) >>> 0] = numBytesWrittenExclNull;
    ...
    function _glGetActiveAttrib(
      ...
      type >>>= 0;
      name >>>= 0;
      return __glGetActiveAttribOrUniform(
        "getActiveAttrib",
        program,
        index,
        bufSize,
    ...
    function _glGetBufferParameteriv(target, value, data) {
      ...
      GROWABLE_HEAP_I32()[(data >>> 2) >>> 0] = GLctx.getBufferParameter(
        target,
        value
      );
    }
    ...
    function _glGetQueryivEXT(target, pname, params) {
      ...
      GROWABLE_HEAP_I32()[(params >>> 2) >>> 0] = GLctx.disjointTimerQueryExt[
        "getQueryEXT"
      ](target, pname);
    }
    ...
    function _glGetSamplerParameterfv(sampler, pname, params) {
      ...
      GROWABLE_HEAP_F32()[(params >>> 2) >>> 0] = GLctx.getSamplerParameter(
        GL.samplers[sampler],
        pname
      );
    }
    ...
    function _glGetSamplerParameteriv(sampler, pname, params) {
      ...
      GROWABLE_HEAP_I32()[(params >>> 2) >>> 0] = GLctx.getSamplerParameter(
        GL.samplers[sampler],
        pname
      );
    }
    ...
    function _glGetShaderiv(shader, pname, p) {
      ...
      if (pname == 35716) {
        ...
        GROWABLE_HEAP_I32()[(p >>> 2) >>> 0] = GLctx.getShaderParameter(
          GL.shaders[shader],
          pname
        );
      }
    ...
    function _glGetTexParameterfv(target, pname, params) {
      ...
      GROWABLE_HEAP_F32()[(params >>> 2) >>> 0] = GLctx.getTexParameter(
        target,
        pname
      );
    }
    ...
    function _glGetTexParameteriv(target, pname, params) {
      ...
      GROWABLE_HEAP_I32()[(params >>> 2) >>> 0] = GLctx.getTexParameter(
        target,
        pname
      );
    }
    var _emscripten_glGetTexParameteriv = _glGetTexParameteriv;
    function _glGetTransformFeedbackVarying(
      program,
      index,
      bufSize,
      length,
    ...
    function _glGetUniformIndices(
      ...
      for (var i = 0; i < uniformCount; i++)
        names.push(
          UTF8ToString(
            GROWABLE_HEAP_I32()[((uniformNames + i * 4) >>> 2) >>> 0]
          )
        );
        ...
        for (var i = 0; i < data.length; i++) {
          switch (type) {
            ...
            case 2:
              ...
              break;
            case 5:
              GROWABLE_HEAP_I32()[((params + i * 4) >>> 2) >>> 0] = Math.fround(
                data[i]
              );
              break;
          }
    ...
    function _glGetVertexAttribPointerv(index, pname, pointer) {
      ...
      GROWABLE_HEAP_I32()[(pointer >>> 2) >>> 0] = GLctx.getVertexAttribOffset(
        index,
        pname
      );
    }
    ...
    function _glTransformFeedbackVaryings(
      ...
      for (var i = 0; i < count; i++)
        vars.push(
          UTF8ToString(GROWABLE_HEAP_I32()[((varyings + i * 4) >>> 2) >>> 0])
        );
    ...
    function _glUniform2fv(location, count, value) {
      ...
      if (count <= 144) {
        ...
        for (var i = 0; i < count; i += 2) {
          view[i] = GROWABLE_HEAP_F32()[((value + 4 * i) >>> 2) >>> 0];
          view[i + 1] =
            GROWABLE_HEAP_F32()[((value + (4 * i + 4)) >>> 2) >>> 0];
        }
      } else {
        var view = GROWABLE_HEAP_F32().subarray(
          (value >>> 2) >>> 0,
    ...
    function _glUniform2iv(location, count, value) {
      ...
      if (count <= 144) {
        ...
        for (var i = 0; i < count; i += 2) {
          view[i] = GROWABLE_HEAP_I32()[((value + 4 * i) >>> 2) >>> 0];
          view[i + 1] =
            GROWABLE_HEAP_I32()[((value + (4 * i + 4)) >>> 2) >>> 0];
        }
      } else {
        var view = GROWABLE_HEAP_I32().subarray(
          (value >>> 2) >>> 0,
      ...
      if (count <= 96) {
        ...
        for (var i = 0; i < count; i += 3) {
          ...
          view[i + 1] =
            GROWABLE_HEAP_F32()[((value + (4 * i + 4)) >>> 2) >>> 0];
          view[i + 2] =
            GROWABLE_HEAP_F32()[((value + (4 * i + 8)) >>> 2) >>> 0];
        }
      } else {
        var view = GROWABLE_HEAP_F32().subarray(
          (value >>> 2) >>> 0,
      ...
      if (count <= 96) {
        ...
        for (var i = 0; i < count; i += 3) {
          ...
          view[i + 1] =
            GROWABLE_HEAP_I32()[((value + (4 * i + 4)) >>> 2) >>> 0];
          view[i + 2] =
            GROWABLE_HEAP_I32()[((value + (4 * i + 8)) >>> 2) >>> 0];
        }
      } else {
        var view = GROWABLE_HEAP_I32().subarray(
          (value >>> 2) >>> 0,
      ...
      if (count <= 72) {
        ...
        for (var i = 0; i < count; i += 4) {
          ...
          view[i + 2] =
            GROWABLE_HEAP_I32()[((value + (4 * i + 8)) >>> 2) >>> 0];
          view[i + 3] =
            GROWABLE_HEAP_I32()[((value + (4 * i + 12)) >>> 2) >>> 0];
        }
      } else {
        var view = GROWABLE_HEAP_I32().subarray(
          (value >>> 2) >>> 0,
      ...
      if (count <= 72) {
        ...
        for (var i = 0; i < count; i += 4) {
          ...
          view[i + 2] =
            GROWABLE_HEAP_F32()[((value + (4 * i + 8)) >>> 2) >>> 0];
          view[i + 3] =
            GROWABLE_HEAP_F32()[((value + (4 * i + 12)) >>> 2) >>> 0];
        }
      } else {
        var view = GROWABLE_HEAP_F32().subarray(
          (value >>> 2) >>> 0,
      ...
      if (count <= 32) {
        ...
        for (var i = 0; i < count; i += 9) {
          ...
          view[i + 3] =
            GROWABLE_HEAP_F32()[((value + (4 * i + 12)) >>> 2) >>> 0];
          view[i + 4] =
            GROWABLE_HEAP_F32()[((value + (4 * i + 16)) >>> 2) >>> 0];
          view[i + 5] =
            GROWABLE_HEAP_F32()[((value + (4 * i + 20)) >>> 2) >>> 0];
          view[i + 6] =
            GROWABLE_HEAP_F32()[((value + (4 * i + 24)) >>> 2) >>> 0];
          view[i + 7] =
            GROWABLE_HEAP_F32()[((value + (4 * i + 28)) >>> 2) >>> 0];
          view[i + 8] =
            GROWABLE_HEAP_F32()[((value + (4 * i + 32)) >>> 2) >>> 0];
        }
      } else {
        var view = GROWABLE_HEAP_F32().subarray(
          (value >>> 2) >>> 0,
    ...
    var formatString = (format, varargs) => {
      ...
      function getNextArg(type) {
        ...
        if (type === "double") {
          ...
          ret = [
            GROWABLE_HEAP_I32()[(argIndex >>> 2) >>> 0],
            GROWABLE_HEAP_I32()[((argIndex + 4) >>> 2) >>> 0],
          ];
          argIndex += 8;
        } else {
          type = "i32";
    ...
    var JSEvents = {
      memcpy(target, src, size) {
        GROWABLE_HEAP_I8().set(
          GROWABLE_HEAP_I8().subarray(src >>> 0, (src + size) >>> 0),
          target >>> 0
        );
      },
    ...
    function fetchCacheData(db, fetch, data, onsuccess, onerror) {
      ...
      var fetch_attr = fetch + 108;
      var destinationPath =
        GROWABLE_HEAP_U32()[((fetch_attr + 64) >>> 2) >>> 0];
    ...
    function _emscripten_start_fetch(
      ...
      var onreadystatechange =
        GROWABLE_HEAP_U32()[((fetch_attr + 48) >>> 2) >>> 0];
      var fetchAttributes =
        GROWABLE_HEAP_U32()[((fetch_attr + 52) >>> 2) >>> 0];
      var fetchAttrSynchronous = !!(fetchAttributes & 64);
      ...
      if (requestMethod === "EM_IDB_STORE") {
        ...
        fetchCacheData(
          Fetch.dbInstance,
          fetch,
          GROWABLE_HEAP_U8().slice(ptr, ptr + size),
          reportSuccess,
          reportError
        );
    ...
    function _emscripten_webgl_do_create_context(target, attributes) {
      ...
      var contextAttributes = {
        ...
        failIfMajorPerformanceCaveat:
          !!GROWABLE_HEAP_I8()[(attributes + 12) >>> 0],
        ...
        proxyContextToMainThread:
          GROWABLE_HEAP_I32()[(attr32 + (28 >> 2)) >>> 0],
        renderViaOffscreenBackBuffer:
          GROWABLE_HEAP_I8()[(attributes + 32) >>> 0],
      };
    ...
    function _emscripten_webgl_get_context_attributes(c, a) {
      ...
      GROWABLE_HEAP_I8()[(a + 24) >>> 0] =
        c.attributes.enableExtensionsByDefault;
      return 0;
    }
    ...
    var WS = {
      ...
      getSocketEvent(socketId) {
        ...
        GROWABLE_HEAP_U32()[(this.socketEvent >>> 2) >>> 0] = socketId;
        return this.socketEvent;
      },
    };
    function _emscripten_websocket_close(socketId, code, reason) {
      if (ENVIRONMENT_IS_PTHREAD)
        return proxyToMainThread(73, 0, 1, socketId, code, reason);
      reason >>>= 0;
    ...
    function _emscripten_websocket_delete(socketId) {
      if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(74, 0, 1, socketId);
      var socket = WS.getSocket(socketId);
      if (!socket) {
        return -3;
      }
    ...
    function _emscripten_websocket_get_ready_state(socketId, readyState) {
      if (ENVIRONMENT_IS_PTHREAD)
        return proxyToMainThread(75, 0, 1, socketId, readyState);
      readyState >>>= 0;
      var socket = WS.getSocket(socketId);
      if (!socket) {
        return -3;
      }
      GROWABLE_HEAP_I16()[(readyState >>> 1) >>> 0] = socket.readyState;
      return 0;
    }
    function _emscripten_websocket_new(createAttributes) {
      if (ENVIRONMENT_IS_PTHREAD)
        return proxyToMainThread(76, 0, 1, createAttributes);
      createAttributes >>>= 0;
      ...
      var url = UTF8ToString(
        GROWABLE_HEAP_U32()[(createAttributes >>> 2) >>> 0]
      );
      var protocols = GROWABLE_HEAP_U32()[((createAttributes + 4) >>> 2) >>> 0];
    ...
    function _emscripten_websocket_send_binary(
      socketId,
      binaryData,
      dataLength
    ) {
      if (ENVIRONMENT_IS_PTHREAD)
        return proxyToMainThread(77, 0, 1, socketId, binaryData, dataLength);
      binaryData >>>= 0;
      var socket = WS.getSocket(socketId);
      if (!socket) {
        return -3;
      ...
      socket.send(
        new Uint8Array(
          GROWABLE_HEAP_U8().subarray(
            binaryData >>> 0,
            (binaryData + dataLength) >>> 0
          )
        )
      );
      return 0;
    ...
    function _emscripten_websocket_send_utf8_text(socketId, textData) {
      if (ENVIRONMENT_IS_PTHREAD)
        return proxyToMainThread(78, 0, 1, socketId, textData);
      textData >>>= 0;
      var socket = WS.getSocket(socketId);
      if (!socket) {
        return -3;
    ...
    function _emscripten_websocket_set_onclose_callback_on_thread(
      ...
      socket.onclose = function (e) {
        ...
        (GROWABLE_HEAP_I8()[(eventPtr + 4) >>> 0] = e.wasClean),
          (GROWABLE_HEAP_I16()[((eventPtr + 6) >>> 1) >>> 0] = e.code),
    ...
    function _emscripten_websocket_set_onmessage_callback_on_thread(
      ...
      socket.onmessage = function (e) {
        ...
        if (isText) {
          ...
          var buf = _malloc(len);
          GROWABLE_HEAP_I8().set(new Uint8Array(e.data), buf >>> 0);
        }
        var eventPtr = WS.getSocketEvent(socketId);
        (GROWABLE_HEAP_U32()[((eventPtr + 4) >>> 2) >>> 0] = buf),
          (GROWABLE_HEAP_I32()[((eventPtr + 8) >>> 2) >>> 0] = len),
          (GROWABLE_HEAP_I8()[(eventPtr + 12) >>> 0] = isText),
          getWasmTableEntry(callbackFunc)(0, eventPtr, userData);
        _free(buf);
      };
      return 0;
    ...
    function _environ_get(__environ, environ_buf) {
      if (ENVIRONMENT_IS_PTHREAD)
        return proxyToMainThread(83, 0, 1, __environ, environ_buf);
      __environ >>>= 0;
      environ_buf >>>= 0;
      var bufSize = 0;
      var envp = 0;
    ...
    function _environ_sizes_get(penviron_count, penviron_buf_size) {
      if (ENVIRONMENT_IS_PTHREAD)
        return proxyToMainThread(84, 0, 1, penviron_count, penviron_buf_size);
      penviron_count >>>= 0;
      penviron_buf_size >>>= 0;
    ...
    function _fd_close(fd) {
      if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(85, 0, 1, fd);
      try {
        var stream = SYSCALLS.getStreamFromFD(fd);
        FS.close(stream);
        return 0;
    ...
    function _fd_fdstat_get(fd, pbuf) {
      if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(86, 0, 1, fd, pbuf);
      pbuf >>>= 0;
      try {
        var rightsBase = 0;
    ...
    function _fd_read(fd, iov, iovcnt, pnum) {
      if (ENVIRONMENT_IS_PTHREAD)
        return proxyToMainThread(87, 0, 1, fd, iov, iovcnt, pnum);
      iov >>>= 0;
      iovcnt >>>= 0;
      pnum >>>= 0;
      try {
    ...
    function _fd_seek(fd, offset, whence, newOffset) {
      if (ENVIRONMENT_IS_PTHREAD)
        return proxyToMainThread(88, 0, 1, fd, offset, whence, newOffset);
      offset = bigintToI53Checked(offset);
      newOffset >>>= 0;
      try {
        if (isNaN(offset)) return 61;
    ...
    function _fd_write(fd, iov, iovcnt, pnum) {
      if (ENVIRONMENT_IS_PTHREAD)
        return proxyToMainThread(90, 0, 1, fd, iov, iovcnt, pnum);
      iov >>>= 0;
      iovcnt >>>= 0;
      pnum >>>= 0;
      try {
    ...
    function _getaddrinfo(node, service, hint, out) {
      if (ENVIRONMENT_IS_PTHREAD)
        return proxyToMainThread(91, 0, 1, node, service, hint, out);
      node >>>= 0;
      service >>>= 0;
      hint >>>= 0;
      out >>>= 0;
    ...
    var ASM_CONSTS = {
      ...
      15616984: ($0) => {
        ...
        let handler = (arg) => {
          control.pendingEvents.push({ index: index, arg: arg });
          if (control.resume) {
      ...
      15617342: () => Module.getRandomValue(),
      15617378: () => {
    ...
    function jsHaveJspi() {
      return (
        ...
        !!Asyncify.makeAsyncFunction &&
        (!!WebAssembly.Function || !!WebAssembly.Suspending)
      );
    }
        ...
        stream = navigator.mediaDevices
          ...
          .then((stream) => {
            ...
            const processFrame = function () {
              ...
              GROWABLE_HEAP_U8().set(sourceBuffer, buffer >>> 0);
    ...
    function assignWasmImports() {
      wasmImports = {
        Ui: ___assert_fail,
        Ba: __asyncjs__qtSuspendJs,
        dc: ___call_sighandler,
        ...
        af: ___cxa_rethrow,
        y: ___cxa_throw,
        gc: ___pthread_create_js,
        c: ___resumeException,
        Sa: ___syscall_accept4,
        Qb: ___syscall_bind,
        Za: ___syscall_chmod,
        Pb: ___syscall_connect,
        Ic: ___syscall_faccessat,
        Dc: ___syscall_fchmod,
        X: ___syscall_fcntl64,
        Cc: ___syscall_fstat64,
        xc: ___syscall_ftruncate64,
        wc: ___syscall_getcwd,
        cc: ___syscall_getdents64,
        Ob: ___syscall_getpeername,
        Nb: ___syscall_getsockname,
        Ea: ___syscall_getsockopt,
        Fa: ___syscall_ioctl,
        Mb: ___syscall_listen,
        Ac: ___syscall_lstat64,
        pc: ___syscall_mkdirat,
        zc: ___syscall_newfstatat,
        Ya: ___syscall_openat,
        jc: ___syscall_pipe,
        ic: ___syscall_poll,
        bc: ___syscall_readlinkat,
        Lb: ___syscall_recvfrom,
        Kb: ___syscall_recvmsg,
        _b: ___syscall_renameat,
        $b: ___syscall_rmdir,
        Jb: ___syscall_sendmsg,
        Ra: ___syscall_socket,
        Bc: ___syscall_stat64,
        Zb: ___syscall_symlinkat,
        Vb: ___syscall_truncate64,
        ac: ___syscall_unlinkat,
        Ub: ___syscall_utimensat,
        Jc: __abort_js,
        nb: __embind_register_bigint,
        Ii: __embind_register_bool,
        sb: __embind_register_class,
        qb: __embind_register_class_class_function,
        rb: __embind_register_class_function,
        Gi: __embind_register_emval,
        mb: __embind_register_float,
        O: __embind_register_function,
        ca: __embind_register_integer,
        P: __embind_register_memory_view,
        Hi: __embind_register_std_string,
        Ha: __embind_register_std_wstring,
        Ji: __embind_register_void,
        Zh: __emscripten_fetch_get_response_headers,
        _h: __emscripten_fetch_get_response_headers_length,
        sc: __emscripten_init_main_thread_js,
        Wb: __emscripten_notify_mailbox_postmessage,
        hc: __emscripten_receive_on_main_thread_js,
        Ta: __emscripten_runtime_keepalive_clear,
        Ua: __emscripten_thread_cleanup,
        rc: __emscripten_thread_mailbox_await,
        Fc: __emscripten_thread_set_strongref,
        Rb: __emscripten_throw_longjmp,
        v: __emval_as,
        Y: __emval_call,
        ...
        p: __emval_incref,
        Fi: __emval_instanceof,
        da: __emval_new_array,
        l: __emval_new_cstring,
        I: __emval_new_object,
        Ei: __emval_not,
        ...
        pa: _alGetSourcei,
        ib: _alGetString,
        ki: _alIsExtensionPresent,
        ni: _alSourcePause,
        za: _alSourcePlay,
        eb: _alSourceQueueBuffers,
        db: _alSourceRewind,
        ti: _alSourceStop,
        cb: _alSourceUnqueueBuffers,
        gb: _alSourcef,
        li: _alcCaptureCloseDevice,
        mi: _alcCaptureOpenDevice,
        bb: _alcCaptureSamples,
        ab: _alcCaptureStart,
        $a: _alcCaptureStop,
        oi: _alcCloseDevice,
        wi: _alcCreateContext,
        pi: _alcDestroyContext,
        ja: _alcGetError,
        Ga: _alcGetIntegerv,
        oa: _alcGetString,
        hb: _alcMakeContextCurrent,
        xi: _alcOpenDevice,
        Hc: _clock_time_get,
        ha: _emscripten_asm_const_int,
        Aa: _emscripten_async_call,
        yi: _emscripten_cancel_animation_frame,
        Va: _emscripten_check_blocking_allowed,
        Gc: _emscripten_date_now,
        Ec: _emscripten_exit_with_live_runtime,
        Yh: _emscripten_fetch_free,
        zi: _emscripten_get_element_css_size,
        Xb: _emscripten_get_heap_max,
        V: _emscripten_get_now,
        Bh: _emscripten_glActiveTexture,
        Ah: _emscripten_glAttachShader,
        we: _emscripten_glBeginQuery,
        Th: _emscripten_glBeginQueryEXT,
        ce: _emscripten_glBeginTransformFeedback,
        zh: _emscripten_glBindAttribLocation,
        yh: _emscripten_glBindBuffer,
        $d: _emscripten_glBindBufferBase,
        ae: _emscripten_glBindBufferRange,
        xh: _emscripten_glBindFramebuffer,
        wh: _emscripten_glBindRenderbuffer,
        gd: _emscripten_glBindSampler,
        vh: _emscripten_glBindTexture,
        _c: _emscripten_glBindTransformFeedback,
        ie: _emscripten_glBindVertexArray,
        Jh: _emscripten_glBindVertexArrayOES,
        uh: _emscripten_glBlendColor,
        th: _emscripten_glBlendEquation,
        sh: _emscripten_glBlendEquationSeparate,
        rh: _emscripten_glBlendFunc,
        qh: _emscripten_glBlendFuncSeparate,
        le: _emscripten_glBlitFramebuffer,
        ph: _emscripten_glBufferData,
        oh: _emscripten_glBufferSubData,
        nh: _emscripten_glCheckFramebufferStatus,
        mh: _emscripten_glClear,
        Dd: _emscripten_glClearBufferfi,
        Ed: _emscripten_glClearBufferfv,
        Gd: _emscripten_glClearBufferiv,
        Fd: _emscripten_glClearBufferuiv,
        lh: _emscripten_glClearColor,
        kh: _emscripten_glClearDepthf,
        jh: _emscripten_glClearStencil,
        pd: _emscripten_glClientWaitSync,
        Qe: _emscripten_glClipControlEXT,
        ih: _emscripten_glColorMask,
        hh: _emscripten_glCompileShader,
        gh: _emscripten_glCompressedTexImage2D,
        Be: _emscripten_glCompressedTexImage3D,
        fh: _emscripten_glCompressedTexSubImage2D,
        Ae: _emscripten_glCompressedTexSubImage3D,
        Bd: _emscripten_glCopyBufferSubData,
        eh: _emscripten_glCopyTexImage2D,
        dh: _emscripten_glCopyTexSubImage2D,
        Ce: _emscripten_glCopyTexSubImage3D,
        ch: _emscripten_glCreateProgram,
        bh: _emscripten_glCreateShader,
        ah: _emscripten_glCullFace,
        $g: _emscripten_glDeleteBuffers,
        _g: _emscripten_glDeleteFramebuffers,
        Zg: _emscripten_glDeleteProgram,
        ye: _emscripten_glDeleteQueries,
        Vh: _emscripten_glDeleteQueriesEXT,
        Yg: _emscripten_glDeleteRenderbuffers,
        id: _emscripten_glDeleteSamplers,
        Wg: _emscripten_glDeleteShader,
        qd: _emscripten_glDeleteSync,
        Vg: _emscripten_glDeleteTextures,
        Zc: _emscripten_glDeleteTransformFeedbacks,
        he: _emscripten_glDeleteVertexArrays,
        Ih: _emscripten_glDeleteVertexArraysOES,
        ...
        ud: _emscripten_glDrawArraysInstanced,
        Eh: _emscripten_glDrawArraysInstancedANGLE,
        Le: _emscripten_glDrawArraysInstancedARB,
        Me: _emscripten_glDrawArraysInstancedEXT,
        Lc: _emscripten_glDrawArraysInstancedNV,
        se: _emscripten_glDrawBuffers,
        He: _emscripten_glDrawBuffersEXT,
        Fh: _emscripten_glDrawBuffersWEBGL,
        Ng: _emscripten_glDrawElements,
        td: _emscripten_glDrawElementsInstanced,
        Dh: _emscripten_glDrawElementsInstancedANGLE,
        Ie: _emscripten_glDrawElementsInstancedARB,
        Je: _emscripten_glDrawElementsInstancedEXT,
        Ke: _emscripten_glDrawElementsInstancedNV,
        Fe: _emscripten_glDrawRangeElements,
        Mg: _emscripten_glEnable,
        Lg: _emscripten_glEnableVertexAttribArray,
        ve: _emscripten_glEndQuery,
        Sh: _emscripten_glEndQueryEXT,
        be: _emscripten_glEndTransformFeedback,
        sd: _emscripten_glFenceSync,
        Kg: _emscripten_glFinish,
        Jg: _emscripten_glFlush,
        Ig: _emscripten_glFramebufferRenderbuffer,
        Hg: _emscripten_glFramebufferTexture2D,
        je: _emscripten_glFramebufferTextureLayer,
        Gg: _emscripten_glFrontFace,
        Fg: _emscripten_glGenBuffers,
        Dg: _emscripten_glGenFramebuffers,
        ze: _emscripten_glGenQueries,
        Xh: _emscripten_glGenQueriesEXT,
        Cg: _emscripten_glGenRenderbuffers,
        jd: _emscripten_glGenSamplers,
        Bg: _emscripten_glGenTextures,
        Yc: _emscripten_glGenTransformFeedbacks,
        ge: _emscripten_glGenVertexArrays,
        Hh: _emscripten_glGenVertexArraysOES,
        Eg: _emscripten_glGenerateMipmap,
        Ag: _emscripten_glGetActiveAttrib,
        zg: _emscripten_glGetActiveUniform,
        wd: _emscripten_glGetActiveUniformBlockName,
        xd: _emscripten_glGetActiveUniformBlockiv,
        zd: _emscripten_glGetActiveUniformsiv,
        yg: _emscripten_glGetAttachedShaders,
        xg: _emscripten_glGetAttribLocation,
        wg: _emscripten_glGetBooleanv,
        kd: _emscripten_glGetBufferParameteri64v,
        vg: _emscripten_glGetBufferParameteriv,
        ug: _emscripten_glGetError,
        tg: _emscripten_glGetFloatv,
        Pd: _emscripten_glGetFragDataLocation,
        sg: _emscripten_glGetFramebufferAttachmentParameteriv,
        ld: _emscripten_glGetInteger64i_v,
        nd: _emscripten_glGetInteger64v,
        ee: _emscripten_glGetIntegeri_v,
        rg: _emscripten_glGetIntegerv,
        Nc: _emscripten_glGetInternalformativ,
        Uc: _emscripten_glGetProgramBinary,
        pg: _emscripten_glGetProgramInfoLog,
        qg: _emscripten_glGetProgramiv,
        Nh: _emscripten_glGetQueryObjecti64vEXT,
        Ph: _emscripten_glGetQueryObjectivEXT,
        Mh: _emscripten_glGetQueryObjectui64vEXT,
        te: _emscripten_glGetQueryObjectuiv,
        Oh: _emscripten_glGetQueryObjectuivEXT,
        ue: _emscripten_glGetQueryiv,
        Qh: _emscripten_glGetQueryivEXT,
        og: _emscripten_glGetRenderbufferParameteriv,
        ad: _emscripten_glGetSamplerParameterfv,
        ...
        ng: _emscripten_glGetShaderiv,
        jg: _emscripten_glGetString,
        Cd: _emscripten_glGetStringi,
        md: _emscripten_glGetSynciv,
        ig: _emscripten_glGetTexParameterfv,
        hg: _emscripten_glGetTexParameteriv,
        Zd: _emscripten_glGetTransformFeedbackVarying,
        yd: _emscripten_glGetUniformBlockIndex,
        Ad: _emscripten_glGetUniformIndices,
        eg: _emscripten_glGetUniformLocation,
        gg: _emscripten_glGetUniformfv,
        fg: _emscripten_glGetUniformiv,
        Qd: _emscripten_glGetUniformuiv,
        Xd: _emscripten_glGetVertexAttribIiv,
        Wd: _emscripten_glGetVertexAttribIuiv,
        bg: _emscripten_glGetVertexAttribPointerv,
        dg: _emscripten_glGetVertexAttribfv,
        cg: _emscripten_glGetVertexAttribiv,
        ag: _emscripten_glHint,
        Rc: _emscripten_glInvalidateFramebuffer,
        Qc: _emscripten_glInvalidateSubFramebuffer,
        $f: _emscripten_glIsBuffer,
        _f: _emscripten_glIsEnabled,
        Zf: _emscripten_glIsFramebuffer,
        Yf: _emscripten_glIsProgram,
        xe: _emscripten_glIsQuery,
        Uh: _emscripten_glIsQueryEXT,
        Xf: _emscripten_glIsRenderbuffer,
        hd: _emscripten_glIsSampler,
        Wf: _emscripten_glIsShader,
        rd: _emscripten_glIsSync,
        Vf: _emscripten_glIsTexture,
        Xc: _emscripten_glIsTransformFeedback,
        fe: _emscripten_glIsVertexArray,
        Gh: _emscripten_glIsVertexArrayOES,
        Uf: _emscripten_glLineWidth,
        Tf: _emscripten_glLinkProgram,
        Wc: _emscripten_glPauseTransformFeedback,
        Sf: _emscripten_glPixelStorei,
        Pe: _emscripten_glPolygonModeWEBGL,
        Rf: _emscripten_glPolygonOffset,
        Re: _emscripten_glPolygonOffsetClampEXT,
        Tc: _emscripten_glProgramBinary,
        Sc: _emscripten_glProgramParameteri,
        Rh: _emscripten_glQueryCounterEXT,
        Ge: _emscripten_glReadBuffer,
        Pf: _emscripten_glReadPixels,
        Of: _emscripten_glReleaseShaderCompiler,
        Nf: _emscripten_glRenderbufferStorage,
        ke: _emscripten_glRenderbufferStorageMultisample,
        Vc: _emscripten_glResumeTransformFeedback,
        Mf: _emscripten_glSampleCoverage,
        dd: _emscripten_glSamplerParameterf,
        cd: _emscripten_glSamplerParameterfv,
        fd: _emscripten_glSamplerParameteri,
        ed: _emscripten_glSamplerParameteriv,
        Lf: _emscripten_glScissor,
        Kf: _emscripten_glShaderBinary,
        Jf: _emscripten_glShaderSource,
        If: _emscripten_glStencilFunc,
        ...
        Ee: _emscripten_glTexImage3D,
        Bf: _emscripten_glTexParameterf,
        Af: _emscripten_glTexParameterfv,
        zf: _emscripten_glTexParameteri,
        yf: _emscripten_glTexParameteriv,
        Pc: _emscripten_glTexStorage2D,
        Oc: _emscripten_glTexStorage3D,
        xf: _emscripten_glTexSubImage2D,
        De: _emscripten_glTexSubImage3D,
        _d: _emscripten_glTransformFeedbackVaryings,
        wf: _emscripten_glUniform1f,
        vf: _emscripten_glUniform1fv,
        uf: _emscripten_glUniform1i,
        tf: _emscripten_glUniform1iv,
        Od: _emscripten_glUniform1ui,
        ...
        Nd: _emscripten_glUniform2ui,
        Jd: _emscripten_glUniform2uiv,
        of: _emscripten_glUniform3f,
        nf: _emscripten_glUniform3fv,
        mf: _emscripten_glUniform3i,
        lf: _emscripten_glUniform3iv,
        Md: _emscripten_glUniform3ui,
        Id: _emscripten_glUniform3uiv,
        kf: _emscripten_glUniform4f,
        jf: _emscripten_glUniform4fv,
        hf: _emscripten_glUniform4i,
        gf: _emscripten_glUniform4iv,
        Ld: _emscripten_glUniform4ui,
        Hd: _emscripten_glUniform4uiv,
        vd: _emscripten_glUniformBlockBinding,
        ff: _emscripten_glUniformMatrix2fv,
        re: _emscripten_glUniformMatrix2x3fv,
        pe: _emscripten_glUniformMatrix2x4fv,
        ef: _emscripten_glUniformMatrix3fv,
        qe: _emscripten_glUniformMatrix3x2fv,
        ne: _emscripten_glUniformMatrix3x4fv,
        df: _emscripten_glUniformMatrix4fv,
        oe: _emscripten_glUniformMatrix4x2fv,
        me: _emscripten_glUniformMatrix4x3fv,
        cf: _emscripten_glUseProgram,
        bf: _emscripten_glValidateProgram,
        $e: _emscripten_glVertexAttrib1f,
        _e: _emscripten_glVertexAttrib1fv,
        Ze: _emscripten_glVertexAttrib2f,
        Ye: _emscripten_glVertexAttrib2fv,
        Xe: _emscripten_glVertexAttrib3f,
        We: _emscripten_glVertexAttrib3fv,
        Ve: _emscripten_glVertexAttrib4f,
        Ue: _emscripten_glVertexAttrib4fv,
        $c: _emscripten_glVertexAttribDivisor,
        Ch: _emscripten_glVertexAttribDivisorANGLE,
        Ne: _emscripten_glVertexAttribDivisorARB,
        Oe: _emscripten_glVertexAttribDivisorEXT,
        Mc: _emscripten_glVertexAttribDivisorNV,
        Vd: _emscripten_glVertexAttribI4i,
        Sd: _emscripten_glVertexAttribI4iv,
        Td: _emscripten_glVertexAttribI4ui,
        Rd: _emscripten_glVertexAttribI4uiv,
        Yd: _emscripten_glVertexAttribIPointer,
        Te: _emscripten_glVertexAttribPointer,
        Se: _emscripten_glViewport,
        od: _emscripten_glWaitSync,
        Bb: _emscripten_idb_delete,
        Db: _emscripten_idb_exists,
        Cb: _emscripten_idb_load,
        Ab: _emscripten_idb_store,
        Ai: _emscripten_is_webgl_context_lost,
        Xg: _emscripten_log,
        Yb: _emscripten_num_logical_cores,
        Vi: _emscripten_pause_main_loop,
        xa: _emscripten_performance_now,
        ji: _emscripten_request_animation_frame_loop,
        Sb: _emscripten_resize_heap,
        Qf: _emscripten_run_script,
        Wi: _emscripten_set_main_loop,
        lb: _emscripten_set_resize_callback_on_thread,
        tb: _emscripten_set_socket_close_callback,
        Ka: _emscripten_set_socket_connection_callback,
        Na: _emscripten_set_socket_error_callback,
        La: _emscripten_set_socket_listen_callback,
        ub: _emscripten_set_socket_message_callback,
        Ma: _emscripten_set_socket_open_callback,
        vb: _emscripten_set_wheel_callback_on_thread,
        $h: _emscripten_start_fetch,
        fc: _emscripten_unwind_to_js_event_loop,
        jb: _emscripten_webgl_create_context,
        Di: _emscripten_webgl_destroy_context,
        Ci: _emscripten_webgl_get_context_attributes,
        Bi: _emscripten_webgl_make_context_current,
        pb: _emscripten_websocket_close,
        ob: _emscripten_websocket_delete,
        ka: _emscripten_websocket_get_ready_state,
        Oi: _emscripten_websocket_new,
        Pi: _emscripten_websocket_send_binary,
        Qi: _emscripten_websocket_send_utf8_text,
        Ki: _emscripten_websocket_set_onclose_callback_on_thread,
        Li: _emscripten_websocket_set_onerror_callback_on_thread,
        Mi: _emscripten_websocket_set_onmessage_callback_on_thread,
        Ni: _emscripten_websocket_set_onopen_callback_on_thread,
        tc: _environ_get,
    ...
    var ___wasm_call_ctors = () => (___wasm_call_ctors = wasmExports["Yi"])();
    var _main = (Module["_main"] = (a0, a1) =>
      (_main = Module["_main"] = wasmExports["_i"])(a0, a1));
    var _free = (a0) => (_free = wasmExports["$i"])(a0);
    var _malloc = (a0) => (_malloc = wasmExports["aj"])(a0);
    var _pthread_self = () => (_pthread_self = wasmExports["bj"])();
    var ___getTypeName = (a0) => (___getTypeName = wasmExports["cj"])(a0);
    var __embind_initialize_bindings = (Module["__embind_initialize_bindings"] =
      () =>
        (__embind_initialize_bindings = Module["__embind_initialize_bindings"] =
          wasmExports["dj"])());
    var _htonl = (a0) => (_htonl = wasmExports["ej"])(a0);
    var _htons = (a0) => (_htons = wasmExports["fj"])(a0);
    var _ntohs = (a0) => (_ntohs = wasmExports["gj"])(a0);
    var __emscripten_tls_init = () =>
      (__emscripten_tls_init = wasmExports["hj"])();
    var _emscripten_builtin_memalign = (a0, a1) =>
      (_emscripten_builtin_memalign = wasmExports["ij"])(a0, a1);
    var __emscripten_run_callback_on_thread = (a0, a1, a2, a3, a4) =>
      (__emscripten_run_callback_on_thread = wasmExports["jj"])(
        a0,
        a1,
        a2,
        a3,
    ...
    var __emscripten_thread_init = (a0, a1, a2, a3, a4, a5) =>
      (__emscripten_thread_init = wasmExports["kj"])(a0, a1, a2, a3, a4, a5);
    var __emscripten_thread_crashed = () =>
      (__emscripten_thread_crashed = wasmExports["lj"])();
    var __emscripten_run_on_main_thread_js = (a0, a1, a2, a3, a4) =>
      (__emscripten_run_on_main_thread_js = wasmExports["mj"])(
        a0,
        a1,
        a2,
        a3,
    ...
    var __emscripten_thread_free_data = (a0) =>
      (__emscripten_thread_free_data = wasmExports["nj"])(a0);
    var __emscripten_thread_exit = (a0) =>
      (__emscripten_thread_exit = wasmExports["oj"])(a0);
    var __emscripten_timeout = (a0, a1) =>
      (__emscripten_timeout = wasmExports["pj"])(a0, a1);
    var __emscripten_check_mailbox = () =>
      (__emscripten_check_mailbox = wasmExports["qj"])();
    var _setThrew = (a0, a1) => (_setThrew = wasmExports["rj"])(a0, a1);
    var __emscripten_tempret_set = (a0) =>
      (__emscripten_tempret_set = wasmExports["sj"])(a0);
    var _emscripten_stack_set_limits = (a0, a1) =>
      (_emscripten_stack_set_limits = wasmExports["tj"])(a0, a1);
    var __emscripten_stack_restore = (a0) =>
      (__emscripten_stack_restore = wasmExports["uj"])(a0);
    var __emscripten_stack_alloc = (a0) =>
      (__emscripten_stack_alloc = wasmExports["vj"])(a0);
    var _emscripten_stack_get_current = () =>
      (_emscripten_stack_get_current = wasmExports["wj"])();
    var ___cxa_decrement_exception_refcount = (a0) =>
      (___cxa_decrement_exception_refcount = wasmExports["xj"])(a0);
    var ___cxa_increment_exception_refcount = (a0) =>
      (___cxa_increment_exception_refcount = wasmExports["yj"])(a0);
    var ___cxa_can_catch = (a0, a1, a2) =>
      (___cxa_can_catch = wasmExports["zj"])(a0, a1, a2);
    var ___cxa_get_exception_ptr = (a0) =>
      (___cxa_get_exception_ptr = wasmExports["Aj"])(a0);
    ...
    function applySignatureConversions(wasmExports) {
      ...
      wasmExports["aj"] = makeWrapper_pp(wasmExports["aj"]);
      wasmExports["bj"] = makeWrapper_p(wasmExports["bj"]);
      wasmExports["cj"] = makeWrapper_pp(wasmExports["cj"]);
      wasmExports["ij"] = makeWrapper_ppp(wasmExports["ij"]);
      wasmExports["vj"] = makeWrapper_pp(wasmExports["vj"]);
      wasmExports["wj"] = makeWrapper_p(wasmExports["wj"]);
      wasmExports["Aj"] = makeWrapper_pp(wasmExports["Aj"]);
      return wasmExports;
    }
